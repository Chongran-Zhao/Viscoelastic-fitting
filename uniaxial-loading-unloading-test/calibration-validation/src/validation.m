function validation(paras, num_eq, num_neq, Ft, time, P_exp)
[mu_eq, m_eq, n_eq, mu_neq, m_neq, n_neq, eta_d] = paras_to_array(paras, num_eq, num_neq);
gamma = zeros(size(P_exp));
...
plot(ax, gamma(1:1:end), P_exp(1:1:end), 'Color', '#58508d', 'Marker', 'o', 'MarkerFaceColor', '#58508d', 'MarkerSize', 8, 'LineStyle', 'none');
hold(ax, 'on');
plot(ax, gamma(1:1:end), P_pre(1:1:end), 'linewidth', 3.0, 'Color', '#58508d', 'LineStyle', '-');
...
l = legend(ax, 'exp $\dot{\gamma}=2\:\mathrm{s}^{-1}$', 'fit $\dot{\gamma}=2\:\mathrm{s}^{-1}$',...
               'location', 'northwest', 'Orientation', 'horizontal');
set(l, 'interpreter', 'latex', 'fontsize', 30, 'box', 'off', 'FontWeight', 'bold', 'FontName', 'Helvetica', 'NumColumns', 2);
