...
lambda_1 = zeros(size(P_exp_1));
lambda_2 = zeros(size(P_exp_2));
lambda_3 = zeros(size(P_exp_3));
lambda_1(:) = Ft_1(1,1,:);
lambda_2(:) = Ft_2(1,1,:);
lambda_3(:) = Ft_3(1,1,:);
P_fit_1 = get_P_ij_list(1, 1, mu_eq, m_eq, n_eq, mu_neq, m_neq, n_neq, eta_d, Ft_1, time_1);
P_fit_2 = get_P_ij_list(1, 1, mu_eq, m_eq, n_eq, mu_neq, m_neq, n_neq, eta_d, Ft_2, time_2);
P_fit_3 = get_P_ij_list(1, 1, mu_eq, m_eq, n_eq, mu_neq, m_neq, n_neq, eta_d, Ft_3, time_3);

figure;
% ax = axes('Position', [0.1 0.4 0.8 0.8], 'Box', 'on');
ax = axes('Box', 'on');

plot(ax, lambda_1(1:1:end), P_exp_1(1:1:end), 'Color', '#003f5c', 'Marker', 'o', 'MarkerFaceColor', '#003f5c', 'MarkerSize', 8, 'LineStyle', 'none');
hold(ax, 'on');
plot(ax, lambda_1(1:1:end), P_fit_1(1:1:end), 'linewidth', 3.0, 'Color', '#003f5c', 'LineStyle', '-');
hold(ax, 'on');
plot(ax, lambda_2(1:1:end), P_exp_2(1:1:end), 'Color', '#58508d', 'Marker', 'o', 'MarkerFaceColor', '#58508d', 'MarkerSize', 8, 'LineStyle', 'none');
hold(ax, 'on');
plot(ax, lambda_2(1:1:end), P_fit_2(1:1:end), 'linewidth', 3.0, 'Color', '#58508d', 'LineStyle', '-');
hold(ax, 'on');
plot(ax, lambda_3(1:1:end), P_exp_3(1:1:end), 'Color', '#bc5090', 'Marker', 'o', 'MarkerFaceColor', '#bc5090', 'MarkerSize', 8, 'LineStyle', 'none');
hold(ax, 'on');
plot(ax, lambda_3(1:1:end), P_fit_3(1:1:end), 'linewidth', 3.0, 'Color', '#bc5090', 'LineStyle', '-');
hold(ax, 'on');
...
% print MSD
% y_location = y_location + delta_y;
% MSD = (get_MSD(P_fit_1, P_exp_1) + get_MSD(P_fit_2, P_fit_2) + get_MSD(P_fit_3, P_exp_3)) / 2.0;
